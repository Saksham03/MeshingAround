...
HRESULT Model::CreateMeshletsFromFile(const wchar_t* filename)
{
    //First lets load the OBJ file and store it in our custom class
    ObjLoader<uint32_t> reader;    
    HRESULT hr = reader.LoadObj(filename);
    ...
    //MeshProcessor stores an internal representation of the OBJ data that would be used to compute meshlets
    ...
    options.UnitScale = 10.f;
    MeshProcessor meshProcessor;
    //Just populating MeshProcessor class attributes with OBJ data
    meshProcessor.Extract(options, reader);

    //self-explanatory I guess
    meshProcessor.MeshletizeUsingOptimizedVertexBufferDataAndGenerateCullData(options);

    meshProcessor.Export(options, m_exportMesh);

    m_meshes.resize(1); 

    //Index Data
    m_meshes[0].IndexSize = m_exportMesh.IndexSize;
    m_meshes[0].IndexCount = m_exportMesh.IndexCount;    
    m_meshes[0].Indices = MakeSpan(m_exportMesh.Indices.data(), m_exportMesh.Indices.size());
    
    //Index Subset Data
    m_meshes[0].IndexSubsets = MakeSpan(m_exportMesh.IndexSubsets.data(), m_exportMesh.IndexSubsets.size());

    //Vertex Data
    m_meshes[0].VertexStrides.push_back(m_exportMesh.Layout[0].Stride); //considering only 1 mesh for now
    Span<uint8_t> verts = MakeSpan(m_exportMesh.Vertices[0].data(), m_exportMesh.Vertices[0].size());
    m_meshes[0].Vertices.push_back(verts);
    m_meshes[0].VertexCount = m_exportMesh.VertexCount;

    m_meshes[0].LayoutElems[0] = c_elementDescs[0]; // Position
    m_meshes[0].LayoutElems[1] = c_elementDescs[1]; // Normal
    m_meshes[0].LayoutDesc.NumElements = 2;
    
    //Meshlet Data
    m_meshes[0].Meshlets = MakeSpan(m_exportMesh.Meshlets.data(), m_exportMesh.Meshlets.size());
    m_meshes[0].MeshletSubsets = MakeSpan(m_exportMesh.MeshletSubsets.data(), m_exportMesh.MeshletSubsets.size());
    m_meshes[0].UniqueVertexIndices = MakeSpan(m_exportMesh.UniqueVertexIndices.data(), m_exportMesh.UniqueVertexIndices.size());
    m_meshes[0].PrimitiveIndices = MakeSpan(m_exportMesh.PrimitiveIndices.data(), m_exportMesh.PrimitiveIndices.size());
