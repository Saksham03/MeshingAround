...
bool MeshProcessor::Process(const ProcessOptions& options, const ObjLoader<uint32_t>& reader, ExportMesh& output)
{
...
bool MeshProcessor::Extract(const ProcessOptions& options, const ObjLoader<uint32_t>& reader)
{
...
void MeshProcessor::MeshletizeUsingOptimizedVertexBufferDataAndGenerateCullData(const ProcessOptions& options)
{
    if (m_indexSize == 4)
    {
        Finalize<uint32_t>(options);
